...
class VanillaSeq2SeqModel(AbstractModel):
    ...
    def __init__(self,
        ...
        with tf.variable_scope(scope or 'seq2seq_graph', dtype=self.dtype):
            ...
            if mode == tf.contrib.learn.ModeKeys.TRAIN:
                ...
                self.sampling_probability = self._get_sampling_probability(params, self.global_step, self.sampling_probability)
    ...
    def __build_encoder(self, params, keep_prob):
        with variable_scope.variable_scope("encoder") as scope:
            ...
            if params.encoder_type == "uni":
                log.print_out("  build unidirectional encoder num_layers = %d" % params.num_layers)
                ...
                log.print_out("  build bidirectional encoder num_layers = %d" % params.num_layers)
    ...
    def __build_decoder_cell(self, params, mode, encoder_outputs, encoder_state, keep_prob):
        cell = rnn_factory.create_cell(params.cell_type, params.hidden_units, self.num_layers,
                                       input_keep_prob=keep_prob,
                                       devices=self.round_robin.assign(self.num_layers))
        ...
        try:
            attn_mechanism = attention_helper.create_attention_mechanism(params.attention_type,
                                                                            params.hidden_units,
    ...
    def __build_decoder(self, params, mode, encoder_outputs, encoder_state, keep_prob):
        iterator = self.iterator
        with variable_scope.variable_scope("decoder") as decoder_scope:
            cell, initial_state = self.__build_decoder_cell(params, mode,
                                                            encoder_outputs, encoder_state,
                                                            keep_prob)
            ...
            if mode != tf.contrib.learn.ModeKeys.INFER:
                ...
                if self.sampling_probability == 0.0:
                    ...
                    helper = tf.contrib.seq2seq.ScheduledEmbeddingTrainingHelper(
                        decoder_emb_inp, iterator.target_sequence_length, self.embeddings, self.sampling_probability)
