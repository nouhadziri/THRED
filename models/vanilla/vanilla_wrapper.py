...
from models import model_helper, ncm_utils
from models.base import NMTEncoderDecoderWrapper
from models.vanilla import eval_metric, vanilla_helper
from models.vanilla import vanilla_model
...
class VanillaNMTEncoderDecoder(NMTEncoderDecoderWrapper):
    ...
    def run_internal_eval(self,
        ...
        if dev_ppl < self.config.best_dev_ppl:
            loaded_eval_model.saver.save(eval_sess,
                                         os.path.join(self.config.best_dev_ppl_dir, 'vanilla.ckpt'),
    ...
    def train(self, target_session="", scope=None):
        ...
        train_model = vanilla_helper.create_train_model(vanilla_model.VanillaSeq2SeqModel, self.config, scope)
        eval_model = vanilla_helper.create_eval_model(vanilla_model.VanillaSeq2SeqModel, self.config, scope)
        ...
        while self.config.epoch < self.config.num_train_epochs and patience > 0:
            ...
            if global_step - last_eval_step >= steps_per_eval:
                ...
                # Evaluate on dev
                self.run_sample_decode(infer_model, infer_sess, model_dir, summary_writer, eval_data)
                dev_ppl, _ = self.run_internal_eval(eval_model, eval_sess, model_dir, summary_writer,
                                                    use_test_set=False)
            ...
            for metric in self.config.metrics:
                ...
                if save_on_best and scores[metric] > getattr(self.config, "best_" + metric):
                    ...
                    model.saver.save(
                        sess,
                        os.path.join(
                            getattr(self.config, "best_" + metric + "_dir"), "vanilla.ckpt"),
    ...
    def test(self):
        ...
        eval_model = vanilla_helper.create_eval_model(vanilla_model.VanillaSeq2SeqModel, self.config)
        with tf.Session(
