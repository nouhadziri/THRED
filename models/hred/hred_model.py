...
from util import log, rnn_factory, vocab
from util.device import DeviceManager, RoundRobin
...
class HierarchichalSeq2SeqModel(AbstractModel):
    ...
    def __init__(self,
        ...
        with tf.variable_scope(scope or 'hred_graph', dtype=dtype):
            self.init_embeddings(params.vocab_file, params.vocab_pkl,
                                 params.embedding_type, self.embedding_size, scope=scope)
            ...
            encoder_results = self.__build_encoder(params,
                                                   encoder_keep_prob, None)
            context_outputs, context_state = self.__build_context(params, encoder_results,
            ...
            logits, sample_id, final_decoder_state = self.__build_decoder(params, mode,
            ...
            if mode == tf.contrib.learn.ModeKeys.TRAIN:
                ...
                self.word_count = sum(
                    [tf.reduce_sum(self.iterator.source_sequence_lengths[t]) for t in range(self.num_turns)]) + \
                                  tf.reduce_sum(self.iterator.target_sequence_length) # to compute the speed of the training
            elif mode == tf.contrib.learn.ModeKeys.EVAL:
                self.eval_loss = loss
    ...
    def __build_encoder(self, params, keep_prob, device):
        ...
        if params.encoder_type == "uni":
            ...
            encoder_cell['uni'] = rnn_factory.create_cell(params.cell_type, params.encoder_hidden_units,
                                                          num_layers=1,
                                                          input_keep_prob=keep_prob,
                                                          devices=[device])
        elif params.encoder_type == "bi":
            log.print_out("  build bidirectional encoder")
            encoder_cell['fw'] = rnn_factory.create_cell(params.cell_type, params.encoder_hidden_units,
                                                         num_layers=1,
                                                         input_keep_prob=keep_prob,
                                                         devices=[device])
            encoder_cell['bw'] = rnn_factory.create_cell(params.cell_type, params.encoder_hidden_units,
                                                         num_layers=1,
                                                         input_keep_prob=keep_prob,
                                                         devices=[device])
        else:
    ...
    def __build_context(self, params, encoder_results, keep_prob, device):
        with variable_scope.variable_scope("context") as scope:
            ...
            with tf.device(device):
                context_inputs = tf.stack([state for outputs, state in encoder_results], axis=0)

                if params.context_type == "uni":
                    cell = rnn_factory.create_cell(params.cell_type, params.decoder_hidden_units,
                                                   num_layers=1,
                                                   input_keep_prob=keep_prob,
                                                   devices=[device])
                    ...
                    fw_cell = rnn_factory.create_cell(params.cell_type, params.decoder_hidden_units,
                                                      num_layers=1,
                                                      input_keep_prob=keep_prob,
                                                      devices=[device])
                    bw_cell = rnn_factory.create_cell(params.cell_type, params.decoder_hidden_units,
                                                      num_layers=1,
                                                      input_keep_prob=keep_prob,
                                                      devices=[device])
    ...
    def __build_decoder(self, params, mode,
                        context_outputs, context_state,
                        input_keep_prob, device):

        iterator = self.iterator

        decoder_cell = rnn_factory.create_cell(params.cell_type, params.decoder_hidden_units,
                                               num_layers=1,
