...
from models.legacy.topic_aware import taware_layer, taware_decoder
...
class TopicAwareHierarchicalSeq2SeqModel(AbstractModel):
    ...
    def __init__(self,
        ...
        with tf.variable_scope(scope or 'thred_graph', dtype=self.dtype):
            ...
            encoder_results = self.__build_encoder(params, encoder_keep_prob, devices[0])
            ...
            logits, sample_ids, final_decoder_state = self.__build_decoder(params, context_outputs, context_state,
        ...
        with variable_scope.variable_scope("context"):
            with tf.device(device):
                ...
                if params.context_direction == 'backward':
                    context_inputs = tf.stack([state for _, state in reversed(encoder_results)], axis=0)
                else:
    ...
    def __build_decoder_cell(self, params, context_outputs, context_state,
        ...
        expanded_context_state = tf.tile(tf.expand_dims(context_state, axis=1), [1, max_topic_length, 1])
