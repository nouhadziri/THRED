...
from models import ncm_utils, model_helper
from util import fs, log, vocab, misc
from util.embed import WordEmbeddings


class AbstractModel(object):

    def init_embeddings(self, vocab_file, vocab_pkl, embedding_type, embedding_size, dtype=tf.float32, scope=None):
        ...
        with tf.variable_scope(scope or "embeddings", dtype=dtype):
            ...
            if embedding_type == 'random':
                ...
                self.embeddings = tf.get_variable("emb_random_mat",
                                                  ...
                                                  initializer=tf.random_uniform_initializer(-sqrt3, sqrt3, dtype))
            else:
                log.print_out('# Using pretrained embedding: %s.' % embedding_type)

                trainable_words, const_words = [], []
                for word in vocab_list:
                    if word not in embed_dict:
                        embed_dict[word] = np.random.normal(0, sqrt3, size=(1, embedding_size)).tolist()[0]
                        trainable_words.append(word)
                    else:
                        const_words.append(word)

                emb_mat = tf.constant(np.array([embed_dict[w] for w in vocab_list], dtype=dtype.as_numpy_dtype()))
                emb_const_mat = tf.slice(emb_mat, [len(trainable_words), 0], [-1, -1])

                with tf.variable_scope(scope or "pretrained_embeddings", dtype=dtype):
                    emb_reserved_mat = tf.get_variable("emb_reserved_mat",
...
class NMTEncoderDecoderWrapper(AbstractEncoderDecoderWrapper):
    def __init__(self, config) -> None:
        ...
        self.config.vocab_pkl = os.path.join(self.config.model_dir,
                                             'vocab_{}d_{}.pkl'.format(self.config.embedding_size, self.config.embedding_type))
    ...
    def _load_data(self, input_file, include_target=False):
        ...
        with codecs.getreader("utf-8")(
            ...
            for line in f.readlines():
                ...
                sources = utterances[:-1]

                if len(sources) == 1:
                    sources = sources[0].strip()
                else:
    ...
    def run_full_eval(self,
                      ...
                      label,
                      summary_writer):
        dev_ppl, test_ppl = self.run_internal_eval(eval_model, eval_sess, model_dir, summary_writer, use_test_set=True)
