...
from thred.models import topical_base, ncm_utils
from thred.models.base import NMTEncoderDecoderWrapper
from thred.models.topic_aware import taware_helper
...
class TopicAwareNMTEncoderDecoder(NMTEncoderDecoderWrapper):
    ...
    def train(self, target_session="", scope=None):
        ...
        train_model = taware_helper.create_train_model(self.config, scope)
        eval_model = taware_helper.create_eval_model(self.config, scope)
        infer_model = taware_helper.create_infer_model(self.config, scope)
    ...
    def _sample_decode(self,
        ...
        sample_data = eval_data[decode_id]

        iterator_feed_dict = {
            iterator_src_placeholder: [sample_data],
        ...
        delimited_sample = sample_data.split("\t")
    ...
    def test(self):
        ...
        infer_model = taware_helper.create_infer_model(self.config)
        ...
        eval_model = taware_helper.create_eval_model(self.config)
        with tf.Session(
                config=model_helper.get_config_proto(self.config.log_device), graph=eval_model.graph) as eval_sess:
