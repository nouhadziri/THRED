...
def _build_halfwidth():
    ...
    for ch in s:
        map[bytes([msb, mb, lsb]).decode('utf-8')] = ch
...
def _build_enclosed_supplements():
    ...
    for n in range(10):
        map[bytes([b3, b2, b1, b0]).decode('utf-8')] = '{},'.format(n)
        b0 += 1
    ...
    for l in capital_alphabet:
        map[bytes([b3, b2, b1, b0]).decode('utf-8')] = '({})'.format(l)
        b0 += 1
        for i, code in enumerate(alphas):
            map[bytes(code).decode('utf-8')] = l
            code[3] += 1
            if code[3] == 0xc0:
                code[3] = 0x80
                code[2] += 1
...
def get_table():
    ...
    map.update(_build_halfwidth())
    map.update(_build_enclosed_supplements())
    map.update(_build_extended_map())
